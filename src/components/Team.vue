<template>
  <tr class="team-line">
      ...
      <td class="name">
        <input type="text" class="team-name" :id="'team-name-' + index" v-model="teamName" @click="select('team-name')" tabindex="1">
      </td>
      <td class="round-score" v-for="(round, roundIndex) in rounds" :key="roundIndex">
        <input 
          type="number" class="score" 
          :id="'score-' + index + '-' + parseInt(roundIndex + 1)" 
          min="-1" max="25" 
          :value="rounds[roundIndex].gained" 
          @change="updateRound" @click="select('score', roundIndex + 1)" 
